...
class ST_GRAPH:
            ...
            for framenum in range(self.seq_length):
                ...
                for ped_in in range(frame.shape[0]):
                    for ped_out in range(ped_in + 1, frame.shape[0]):
                        ...
                        pos_in = (frame[ped_in, 1], frame[ped_in, 2], frame[ped_in, 4])
                        pos_out = (frame[ped_out, 1], frame[ped_out, 2], frame[ped_in, 4])
                        pos = (pos_in, pos_out)
    ...
    def getSequence(self):
        ...
        retEdges = np.zeros(
            (self.seq_length, numNodes * numNodes, 3)
        ...
        return retNodes, retEdges[:, :, 0:2], retNodePresent, retEdgePresent#此处retEdges只返回retEdges[:,:,0:2]
