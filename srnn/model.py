...
class NodeRNN(nn.Module):
    """
    Class representing Agent Node RNNs in the st-graph
    """
    ...
    def __init__(self, args, infer=False):
        ...
        self.output_size = args.node_output_size  # 13(分类预测交互类型)，现改为3（预测位置和航向）
    ...
    def forward(self, state, node_type, h_temporal, h_spatial_other, h, c):
        """
        Forward pass for the model
        params:
        state : input state  #[num_nodes,3]
        h_temporal : hidden state of the temporal edgeRNN corresponding to this node #  [num_nodes,256]
        h_spatial_other : output of the attention module    # [num_nodes, 256]
        h : hidden state of the current nodeRNN    #[num_nodes,128]
        c : cell state of the current nodeRNN    #[num_nodes,128]
        """
        # Encode the input state
        new_state = torch.cat(
            (state, (torch.ones(state.shape[0], 1) * node_type).cuda()), dim=1
        )
        encoded_input = self.encoder_linear(new_state)
...
class EdgeRNN(nn.Module):
    """
    Class representing the Agent-Agent Edge RNN in the s-t graph
    """
...
class EdgeAttention(nn.Module):
    ...
    def forward(self, h_temporal, h_spatials):
        ...
        weighted_value = torch.mv(torch.t(h_spatials), attn)#torch.t()是转置,torch.mv()是乘法
...
class SRNN(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        self.infer = infer
        self.use_cuda = args.use_cuda
        self.obs_length = args.seq_length - args.pred_length  # 自动计算为31
