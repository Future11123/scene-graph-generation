...
def sample_gaussian_2d(outputs, nodesPresent):
    """
...
#    next_y = torch.zeros(numNodes)
    next_inter = torch.zeros(numNodes)
    for node in range(numNodes):
        if node not in nodesPresent:
            continue
        pred = outputs[node, :]
        pred = pred.view(1,-1)
        next_inter[node] = torch.max(pred, 1)[1]
...
#        counter_car = 0
        for ped_nodeID in assumed_ped_NodesPresent:
            if ped_nodeID not in true_ped_NodesPresent[tstep]:
                continue
            pred_pos_ped = ret_nodes[tstep, ped_nodeID, 2]
            true_pos_ped = nodes[tstep, ped_nodeID, 2]
            acc_ped += (pred_pos_ped == true_pos_ped).data.cpu().item()
            ped_counter += 1
#        if counter_ped != 0:
#            acc_ped = acc_ped + (error_ped[tstep].item() / counter_ped)
#            ped_counter += 1

        for bic_nodeID in assumed_bic_NodesPresent:
            if bic_nodeID not in true_bic_NodesPresent[tstep]:
                continue
            pred_pos_bic = ret_nodes[tstep, bic_nodeID, 2]
            true_pos_bic = nodes[tstep, bic_nodeID, 2]
            acc_bic += (pred_pos_bic == true_pos_bic).data.cpu().item()
            bic_counter += 1
#        if counter_bic != 0:
#            acc_bic = acc_bic + (error_bic[tstep].item() / counter_bic)
#            bic_counter += 1

        for car_nodeID in assumed_car_NodesPresent:
            if car_nodeID not in true_car_NodesPresent[tstep]:
                continue
            pred_pos_car = ret_nodes[tstep, car_nodeID, 2]
            true_pos_car = nodes[tstep, car_nodeID, 2]
            acc_car += (pred_pos_car == true_pos_car).data.cpu().item()
            car_counter += 1
#        if counter_car != 0:
#            acc_car = acc_car + (error_car[tstep].item() / counter_car)
#            car_counter += 1

    return acc_ped, acc_bic, acc_car, ped_counter, bic_counter, car_counter
