...
class ST_GRAPH:
    def __init__(self, batch_size=50, seq_length=10):
        ...
        """
        对应一个batch中每个样本（一个样本是seq长度的一系列帧）设置一个字典，字典中的键为一个样本中
        的所有agent的ID（self.edges中则是所有edge的ID）
        """
    ...
    def readGraph(self, source_batch):#source_batch就是数据处理里面得到的一个batch的x
        """
        ...
        """
        for sequence in range(self.batch_size):#sequence代表的是一个batch里的第几个样本
            ...
            for framenum in range(self.seq_length):
                ...
                for ped in range(frame.shape[0]):
                    ...
                    lane_type = frame[ped, 5]
                    lane_width = frame[ped, 6]
                    road_mark_type = frame[ped, 7]
                    distance = frame[ped, 8]
                    # inter = frame[ped, 4]
                    pos = (x, y, heading, lane_type, lane_width, road_mark_type, distance)#此处把heading加进去


                    #如果这个ID在这个长度为seq_length的序列里是第一次出现，就新建一个node
                    if pedID not in self.nodes[sequence]:
                        ...
                        #添加edge隶属于遍历一帧中的某个ID（也就是说注意理解此处缩进关系）
                        edge_id = (pedID, pedID)    # 时间边标识（同一实体）
                        pos_edge = (
                ...
                for ped_in in range(frame.shape[0]):
                    for ped_out in range(ped_in + 1, frame.shape[0]):
                        ...
                        pos_in = (frame[ped_in, 1], frame[ped_in, 2], frame[ped_in, 3], frame[ped_in, 5], frame[ped_in, 6], frame[ped_in, 7], frame[ped_in, 8])
                        pos_out = (frame[ped_out, 1], frame[ped_out, 2], frame[ped_out, 3], frame[ped_out, 5], frame[ped_out, 6], frame[ped_out, 7], frame[ped_out, 8])
                        pos = (pos_in, pos_out)
    ...
    def getSequence(self):
        ...
        retEdges = np.zeros(
            (self.seq_length, numNodes * numNodes, 7)
        ...
        for i, ped in enumerate(nodes.keys()):
            ...
            for framenum in range(self.seq_length):
                if framenum in pos_list:
                    ...
                    retNodes[framenum, i, :] = list(pos_list[framenum])#可以这样用list给矩阵赋值
        ...
        return retNodes, retEdges[:, :, 0:7], retNodePresent, retEdgePresent   #此处retEdges只返回retEdges[:,:,0:2],现在全部加上
...
class ST_NODE:
    def __init__(self, node_type, node_id, node_pos_list):
        ...
        self.node_pos_list = node_pos_list#是个字典，键是一个node出现的帧，值是其位置
    ...
    def getPosition(self, index_i):
        ...
        if index_i not in self.node_pos_list:
            ...
            return self.node_pos_list[last_index]#如果index_i指向的帧里没有这个node，就去更前面找到最近含有这个node的一帧
