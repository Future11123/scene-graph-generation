...
def main():
    ...
    parser.add_argument(
        "--node_input_size", type=int, default=8, help="Dimension of the node features"
    )
    parser.add_argument(
        "--edge_input_size",
        type=int,
    ...
    parser.add_argument(
        "--pred_length", type=int, default=1, help="Predicted sequence length"
    )
...
def train(args):
    ...
    dataloader = DataLoader(args.batch_size, args.seq_length + 1, forcePreProcess=True)
...
#    best_val_loss = 100
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            start = time.time()
            # Get batch data 将x_batch,y_batch,frame_batch,d进行赋值
...
#            print(dataloader.data[dataloader.dataset_pointer])
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # Compute loss 传入损失函数的东西要变   改为0：3代表前三个特征
                # print(f"outputs shape: {outputs.shape}")
                # print(f"nodes[1:, :, 0:3] shape: {nodes[1:, :, 0:3].shape}")
                # print(f"nodesPresent[1:] length: {len(nodesPresent[1:])}")
                # print(f"args.pred_length: {args.pred_length}")

                loss = Gaussian2DLikelihood(
                    outputs, nodes[1:, :, 0:2], nodesPresent[1:], args.pred_length
                )
