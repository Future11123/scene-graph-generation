...
class NodeRNN(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        self.output_size = args.node_output_size  # 13 改为3
    ...
    def forward(self, pos, node_type, h_temporal, h_spatial_other, h, c):
        ...
        # print(f"new_pos shape: {new_pos.shape}")  # 打印 new_pos 的形状
        # print(f"encoder_linear.weight shape: {self.encoder_linear.weight.shape}")  # 打印线性层权重的形状
...
class SRNN(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        self.edge_rnn_size = args.edge_rnn_size
        self.output_size = args.node_output_size  # 13
        ...
        print("linear_embeding:", self.linear_embeding.in_features, "→", self.linear_embeding.out_features)
        print("output_layer:", self.output_layer.in_features, "→", self.output_layer.out_features)
        ...
        for framenum in range(self.seq_length):
            ...
            if len(nodeIDs) != 0:
                ...
                list_of_nodes_car = Variable(
                    torch.LongTensor([x[0] for x in nodeIDs if int(x[1]) == 0])
                ).cuda()
